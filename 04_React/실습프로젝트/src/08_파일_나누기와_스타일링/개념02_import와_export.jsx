...
// 그것을 푸는 문법이 이 둘입니다.
//
// ★ import 는 사실 01단원부터 계속 봐 왔습니다.
//   파일 맨 위에 늘 이런 줄이 있었습니다.
//
//       import { useState } from "react";
//       import Summary from "../_ui/Summary.jsx";
//
//   지금까지는 "그냥 있는 줄" 이었습니다. 이 파일에서 그 줄의 정체를 봅니다.
//   왜 필요한지, 중괄호는 언제 붙이는지, 경로는 어떻게 읽는지 — 섹션 5까지 가면 다 나옵니다.
//
// ★ 이 파일 맨 위(바로 아래)에 import 줄이 여러 개 있습니다.
//   그게 오늘의 주제입니다. 읽으면서 하나씩 돌아와 보세요.
//
// ★ 콘솔에 같은 줄이 두 번씩 찍힙니다. 정상입니다.
//   main.jsx 의 StrictMode 때문입니다(09단원 개념02 섹션1). 여러분이 잘못한 게 아닙니다.
...
//
// 그런데 이렇게 만든 MenuRow 는 그 파일 안에서만 삽니다.
// 03단원에서 만든 것을 05단원에서 또 쓰고 싶어도 부를 방법이 없어서,
...
//
// ./ 는 '개념02 파일이 들어 있는 폴더', 즉 08_파일_나누기와_스타일링 폴더입니다.
...
//
// 둘 다 "여러 개 중에서 이 이름을 골라 꺼낸다" 는 뜻이라 모양이 닮았습니다.
// 하지만 완전히 다른 문법입니다. 헷갈리지 않게 차이를 적어 둡니다.
//
//   const { useState } = 어떤객체;
...
//
// 아래에서 확인해 봅시다. 04단원에서 만든 카운터와 한 글자도 안 다릅니다.
// 그때도 지금도 맨 위의 import 줄이 useState 를 가져다 준 것입니다.
  ...
  return (
    <div>
      ...
      <p className="guide">
        ...
        많습니다. 같은 줄이 <strong>두 번씩</strong> 찍히는 것은 정상입니다(09단원 개념02의
        StrictMode).
        <br />
        <br />
        이 파일이 쓰는 부품은 <code>src/08_파일_나누기와_스타일링/_부품/</code> 안에 있습니다. 그
      ...
      <Summary
        items={[
          ...
          "import { useState } 의 중괄호는 JS자료 09단원의 객체 구조분해와 닮았지만 다른 문법입니다. 구조분해는 함수 안에서도 되지만 import 는 파일 맨 위에만 됩니다.",
...
//    //       Failed to resolve import "./_ui/Summary.jsx" from "src/08_파일_나누기와_스타일링/개념02_import와_export.jsx"
