...
console.log(`${students[0].name} 총점 ${getTotal(students[0].scores)}`);
// ───── 문제 3 ───── 평균 구하는 함수
...
function getAverage(scores) {
  return getTotal(scores) / subjects.length;
}
console.log(
  `${students[1].name} 평균 ${getAverage(students[1].scores).toFixed(1)}`,
);
// ───── 문제 4 ───── 등급 정하는 함수
...
students.forEach((student) => {
  ...
  console.log(
    `${student.name} ${getTotal(student.scores)}점 평균 ${average.toFixed(1)} (${getGrade(average)})`,
  );
});
// ───── 문제 6 ───── 합격자만
...
students.forEach((student) => {
  if (getAverage(student.scores) > getAverage(highStudent.scores)) {
    highStudent = student;
  }
});

console.log(
  `1등: ${highStudent.name} (${getAverage(highStudent.scores).toFixed(1)})`,
);
// ───── 문제 8 ───── 과목별 평균
